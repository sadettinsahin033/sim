...
namespace Gley.PedestrianSystem
{
    ...
    /// </summary>
    public class PedestrianWaypointManager : IDestroyable
    {
        ...
        /// </summary>
        public class PositionWaypoint
        {
            public int WaypointIndex;
            public Vector3 WaypointPosition;

            public PositionWaypoint(int waypointIndex, Vector3 waypointPosition)
            {
        ...
        public class MovementParams
        {
            ...
            public List<PositionWaypoint> GetPath()
            {
                return _targetWaypoints;
            }


            public void SetTargetWaypoints(List<PositionWaypoint> path)
            {
                _targetWaypoints = path;
            }
            ...
            public void DontRemovePedestrian(bool dontRemove)
            {
        ...
        internal PedestrianWaypointManager(int maxNumberOfPedestrians, GridData gridData, PedestrianWaypointsDataHandler pedestrianWaypointsDataHandler)
        {
            ...
            Assign();
            SharedPedestrianEvents.OnStopStateChanged += StopStateChanged;
        }
        ...
        public Dictionary<int, MovementParams> GetMovementParams()
        {
        ...
        public bool CanBeRemoved(int pedestrianIndex)
        {
            if (IsPedestrianIndexValid(pedestrianIndex))
            {
                return !_movementParams[pedestrianIndex].DontRemove;
            }
            return true;
        }
        ...
        internal void RemovePedestrian(int pedestrianIndex)
        {
            if (IsPedestrianIndexValid(pedestrianIndex))
            {
                ...
                if (!IsJustPosition(waypointIndex))
                {
                    if (_pedestrianWaypointsDataHendler.HasIntersection(waypointIndex))
                    {
                        var intersections = _pedestrianWaypointsDataHendler.GetAssociatedIntersection(waypointIndex);
                        foreach (var intersection in intersections)
                        {
                            intersection.PedestrianPassed(pedestrianIndex);
                        }
                    }
                }
        ...
        internal void SetInitialWaypoint(int pedestrianIndex, int waypointIndex, PedestrianTypes type, float offset)
        {
            if (IsPedestrianIndexValid(pedestrianIndex))
            {
                ...
                _movementParams[pedestrianIndex].DontRemovePedestrian(false);
        ...
        /// <param name="pedestrianIndex"></param>
        /// <returns></returns>
        internal Quaternion GetCurrentWaypointRotation(int currentWaypointIndex, int nextWaypointIndex)
        {
            return Quaternion.LookRotation(_pedestrianWaypointsDataHendler.GetPosition(nextWaypointIndex) - _pedestrianWaypointsDataHendler.GetPosition(currentWaypointIndex));
        }
