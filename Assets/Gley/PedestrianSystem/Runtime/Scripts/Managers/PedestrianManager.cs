...
namespace Gley.PedestrianSystem
{
    ...
    public class PedestrianManager : MonoBehaviour
    {
        ...
        public AllPedestrians AllPedestrians
        {
            get
            {
        ...
        public PedestrianWaypointManager WaypointManager
        {
            get
            {
        ...
        internal void Initialize(Transform[] activeCameras, int maxNumberOfPedestrians, PedestrianPool pedestrianPool, PedestrianSystemOptions options)
        {
            ...
            PedestrianBridgeRegistry.Register(PedestrianWaypointsDataHandler, new PedestrianBridge());
        ...
        private void Update()
        {
            ...
            if (_pedestrianReadyToRemove[_indexToRemove] == true)
            {
                if (AllPedestrians.IsActive(_indexToRemove))
                {
                    ...
                    {
                        RemovePedestrian(_indexToRemove, false);
        ...
        internal void SetDestination(int pedestrianIndex, Vector3 position)
        {
            if (PathFindingManager == null)
                return;
            var currentWaypointIndex = WaypointManager.GetCurrentWaypointIndexFromPath(pedestrianIndex);
            if (currentWaypointIndex == PedestrianSystemConstants.INVALID_WAYPOINT_INDEX)
            {
                currentWaypointIndex = WaypointManager.GetPreviousWaypointIndexFromPath(pedestrianIndex);
            }
            var path = PathFindingManager.GetPathToDestination(pedestrianIndex, currentWaypointIndex, position, AllPedestrians.GetPedestrianType(pedestrianIndex));
        ...
        private void RemovePedestrian(int index, bool force)
        {
            if (!_initialized)
                return;
            DensityManager.RemovePedestrian(_pedestrianListIndex[index], force);
        }
