...
namespace Gley.PedestrianSystem
{
    ...
    internal class PedestrianDensityManager
    {
        ...
        /// Remove a pedestrian.
        /// </summary>
        internal void RemovePedestrian(int pedestrianIndex, bool force)
        {
            if (force == false && !_waypointManager.CanBeRemoved(pedestrianIndex))
            {
                return;
            }
        ...
        private void InstantiatePedestrian(PedestrianRequest request)
        {
            ...
            var currentWaypointIndex = _waypointManager.GetCurrentWaypointIndexFromPath(request.Pedestrian.PedestrianIndex);
            int nextWaypointIndex;
            if (request.Path != null && request.Path.Count > 0)
            {
                nextWaypointIndex = request.Path[0];
            }
            else
            {
                nextWaypointIndex = _waypointManager.GetNextWaypointIndexFromPath(request.Pedestrian.PedestrianIndex);
            }
            _allPedestrians.ActivatePedestrian(request.Pedestrian, _waypointManager.GetCurrentWaypointPositionFromPath(request.Pedestrian.PedestrianIndex), _waypointManager.GetCurrentWaypointRotation(currentWaypointIndex, nextWaypointIndex));
