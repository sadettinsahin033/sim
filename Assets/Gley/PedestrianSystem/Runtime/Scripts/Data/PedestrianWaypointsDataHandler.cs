...
namespace Gley.PedestrianSystem
{
    ...
    public class PedestrianWaypointsDataHandler : IPedestrianWaypointsDataHandler
    {
        ...
        {
            HashSet<int> possibleWaypoints = new HashSet<int>();

            if (HasPrev(waypointIndex))
            {
                possibleWaypoints.UnionWith(GetPrevsWithConditions(waypointIndex, pedestrianType));
            }

            if (HasNeighbors(waypointIndex))
            {
                possibleWaypoints.UnionWith(GetNeighborsWithConditions(waypointIndex, pedestrianType));
            }

            return possibleWaypoints.ToList();
        }
        ...
        internal List<IIntersection> GetAssociatedIntersection(int waypointIndex)
        {
        ...
        {
            if(GetWaypoint(waypointIndex).AssociatedIntersection == null)
            {
                return false;
            }
            return GetWaypoint(waypointIndex).AssociatedIntersection.Count > 0;
        }
