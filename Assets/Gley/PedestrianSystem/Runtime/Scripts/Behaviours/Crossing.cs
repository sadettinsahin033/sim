...
namespace Gley.PedestrianSystem
{
    ...
    public class Crossing : PedestrianBehaviour
    {
        ...
        private List<IIntersection> _intersection;
        ...
        internal void SetIntersection(List<IIntersection> crossingIntersection, int waypointIndex)
        {
        ...
        private void WaypointChanged(int pedestrianIndex, Vector3 newPosition)
        {
            ...
            if (PedestrianIndex == pedestrianIndex)
            {
                ...
                int previousWaypointIndex = API.GetPreviousWaypointIndex(PedestrianIndex);

                if (previousWaypointIndex >= 0 && previousWaypointIndex != _currentWaypoint)
                {
                    var previousWaypoint = API.GetWaypointFromIndex(previousWaypointIndex);
                    var previousIntersections = previousWaypoint?.AssociatedIntersection;

                    if (previousIntersections != null && _intersection != null)
                    {
                        foreach (var intersection in previousIntersections)
                        {
                            if (_intersection.Contains(intersection))
                            {
                                Stop();
                                break;
                            }
