...
{
    public class PedestrianPathDrawer : UrbanSystem.Editor.RoadDrawer<PedestrianPathEditorData,PedestrianPath>
    {
        ...
        public override Vector3 DrawHandle(Path path, int i, RoadBase road, float handleSize)
        {
            ...
            if (i == 0)
            {
                return Handles.FreeMoveHandle(UrbanSystemConstants.PATH_ID, path.GetPoint(i, road.positionOffset), Quaternion.identity, handleSize, Vector2.zero, Handles.SphereHandleCap);
            }
            else
            {
            ...
            if (i == 0)
            {
                return Handles.FreeMoveHandle(UrbanSystemConstants.PATH_ID, path.GetPoint(i, road.positionOffset), handleSize, Vector2.zero, Handles.SphereHandleCap);
            }
            else
            {
                return base.DrawHandle(path, i, road, handleSize);
