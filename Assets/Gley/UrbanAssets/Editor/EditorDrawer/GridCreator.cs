...
namespace Gley.UrbanSystem.Editor
{
    public class GridCreator
    {
        ...
        public GridCreator(GridEditorData gridData)
        {
            _gridData = gridData;
        }


        public void GenerateGrid(int gridCellSize, LayerMask roadLayers)
        {
            ...
            int nrOfRows;
            Bounds b = new Bounds();
            foreach (Collider r in MonoBehaviourUtilities.FindObjectsByType<Collider>(FindObjectsSortMode.None))
            {
            ...
            foreach (Terrain t in MonoBehaviourUtilities.FindObjectsByType<Terrain>(FindObjectsSortMode.None))
            {
                ...
                {
                    if ((roadLayers.value & (1 << t.gameObject.layer)) != 0)
                    {
